...
echo "myip=`curl -s http://whatismyip.host/ | grep -e ipaddress | grep -v N/A | sed -e 's/.*>\(.*\)<.*/\1/'`;
...
mysql -u wordpress_user -p94-fP+*.pJ -h db.demo.carp.telecom-sudparis.eu -D wp -e \"UPDATE wp_postmeta SET meta_value = replace(meta_value, 'http://157.129.233.29/','http://$myip/');\"; " >> /etc/rc.local && chmod +x /etc/rc.local && systemctl enable rc-local.service;
